...
def toh():
    ...
    # initializing Q matrix with all 0s
    Q = [[0 for column in range(num_states)] for row in range(num_states)]
    ...
    for x in range(episodes):
        ...
        # choosing action following epsilon-greedy policy
        if np.random.uniform() <= 0.05:
            ...
            find_max = [0] * j
            for t in range(j):
                find_max[t] = Q[current_state][actions[t]]
            max_qa = max(find_max)
                
            j1 = 0
            actions1 = [0] * 3
            for i in actions[0:j]:
                if (Q[current_state][i] == max_qa):
                    actions1[j1] = i
                    j1 += 1
        ...
        while current_state != fin_state:
            ...
            if np.random.uniform() <= 0.05:
                ...
                find_max = [0] * j
                for t in range(j):
                    find_max[t] = Q[next_state][actions[t]]
                max_qa = max(find_max)
                
                j1 = 0
                actions1 = [0] * 3
                for i in actions[0:j]:
                    if (Q[next_state][i] == max_qa):
                        actions1[j1] = i
                        j1 += 1
                next_state1 = actions1[np.random.randint(0,j1)]
                
            # updating Q, state and action 
            q = Q[next_state][next_state1]
            Q[current_state][next_state]= Q[current_state][next_state] + alpha * (R[current_state][next_state] + gamma * q -  Q[current_state][next_state])
    ...
    while state != fin_state:
        ...
        for t in range(num_states):
            if R[state][t]!= float('-inf'):
                find_max[j] = Q[state][t]
                j += 1
        max_qa = max(find_max)
        j1 = 0
        max_actions = [0] * 3
        for i in range(num_states):
            if Q[state][i] == max_qa and R[state][i]!= float('-inf'):
                max_actions[j1] = i
                j1 += 1
